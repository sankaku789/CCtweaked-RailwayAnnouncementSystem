...
function Player:playSegments(segments, priority, onAudioStarted)
    ...
    -- Shared guidance is generated by every computer's scheduler, but only the
    -- auto-elected owner may enter FCFS. Non-owners fail fast without queuing a
    -- shared speaker request, so the bell can never starve normal announcements.
    if self:_isGuidancePlayback(segments, self.currentPriority) and not self:_mayPlayGuidance() then
